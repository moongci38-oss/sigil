#!/usr/bin/env python3
"""weekly.py — 주간 리서치 메인 오케스트레이터

역할: collector.py를 실행하여 raw-data.json을 생성하고,
Claude Code CLI에 /weekly-research 커맨드를 전달할 준비를 한다.
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Weekly Research 오케스트레이터")
    parser.add_argument("date", nargs="?", help="리포트 기준 날짜 (YYYY-MM-DD). 미입력 시 오늘.")
    ...
    raw_json = BUSINESS_DIR / "01-research" / "weekly" / target.isoformat() / "raw-data.json"
    ...
    if raw_json.exists() and not args.force_collect:
        ...
        if result.returncode != 0:
            print(f"[orchestrator] WARN: collector.py 실패 (exit {result.returncode}). "
                  f"Claude가 런타임에 모든 소스를 직접 검색합니다.", file=sys.stderr)
    ...
    print(f"[orchestrator] 리포트 날짜: {target}")

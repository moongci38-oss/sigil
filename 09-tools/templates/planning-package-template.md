...
| 2 | 개발 계획 | `{folderMap.product}/{project}/YYYY-MM-DD-s4-development-plan.md` | ⬜ |
| 3 | UI/UX 기획서 | `{folderMap.design}/{project}/YYYY-MM-DD-s4-uiux-spec.md` | ⬜ |

### 관리자 산출물 (해당 시)

| # | 산출물 | 파일 경로 | 상태 |
|:-:|--------|---------|:----:|
| 4 | 관리자 상세 기획서 | `{folderMap.product}/{project}/YYYY-MM-DD-s4-admin-detailed-plan.md` | ⬜ |
| 5 | 관리자 UI/UX 기획서 | `{folderMap.design}/{project}/YYYY-MM-DD-s4-admin-uiux-spec.md` | ⬜ |

> 개발 계획(#2)은 서비스+관리자 통합 문서로 작성한다.
...
> 기술 스택, 아키텍처, 개발 환경, ADR + **Trine 세션 로드맵** + **테스트 전략**
...
> 프로젝트 전역 테스트 방침. 개별 Spec의 테스트 섹션은 이 전략을 기반으로 작성한다.
> E2E 시나리오 상세는 S4에서 작성하지 않는다 — Trine Spec Section 10에서만 작성한다.
...
- [ ] 상세 기획서 완성 (화면별 동작, 데이터 흐름, 사이트맵)
- [ ] 개발 계획 완성 (기술 스택, C4 아키텍처, ADR, 로드맵, WBS, 테스트 전략)
...
- [ ] **개발 계획 내 테스트 전략 섹션 완성** (테스트 계층, 시딩, FE/BE 도구, 커버리지 목표)
...
- [ ] 기획 패키지 3대 산출물 → Trine Phase 1 입력

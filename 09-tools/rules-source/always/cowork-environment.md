...
| `mcp__playwright__*` | WebFetch, WebSearch, Claude in Chrome |
| `mcp__sequential-thinking__*` | Task(Plan agent) + TodoWrite |
| `mcp__notion__*` | Notion 커넥터 플러그인 |

## Agent Teams → Task 도구 매핑

| Claude Code | Cowork |
|------------|--------|
| Fan-out/Fan-in | Task 다중 호출 (동시) |
| Pipeline | Task 순차 호출 |
| Competing Hypotheses | Task 병렬 → 비교 |

## Cowork 행동 가이드

- 기술 용어 대신 일반 용어로 설명
- 병렬 작업 자동 판단 — "Agent Teams 사용할까요?" 묻지 않음

...
audience: all
impactDescription: "필수 산출물 4종 미완료 시 Trine 진입 불가. 관리자 산출물 누락 시 개발 단계에서 추가 기획 필요 → 일정 지연"
enforcement: rigid
---
...
| 산출물 | 내용 | Trine 참조 |
|--------|------|-----------|
| **상세 기획서** | 화면별 동작 + 데이터 흐름 + 사이트맵(네비게이션 계층) | Phase 2 Spec |
| **개발 계획** | 기술 스택 + 아키텍처 + ADR + 세션 로드맵 + 마일스톤 + WBS | Phase 1 컨텍스트 |
| **UI/UX 기획서** | 와이어프레임 + 컴포넌트 스펙 + 인터랙션 패턴 + 디자인 가이드 | Phase 2 Spec UI |
| **테스트 전략서** | 테스트 계층/비율 + FE/BE 도구 + 시딩 전략 + 커버리지 목표 | Phase 3 Check |
...
- 필수 산출물 4종 중 하나라도 빠진 상태로 게이트를 통과하지 않는다
...
- **IRON-1**: 필수 산출물 4종이 모두 완성되기 전에 [STOP] Gate를 통과하지 않는다
...
|-------------------|---------------|
| "테스트 전략서는 나중에 작성해도 될 것 같다" | Trine에서 테스트 전략 없이 개발을 시작하면 테스트 부채가 누적된다. 4종 모두 완성이 Gate 통과 조건이다 |

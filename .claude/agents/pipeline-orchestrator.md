...
[STOP] 게이트에서 Human 승인을, [AUTO-PASS] 게이트에서 자동 검증+알림을 관리한다.
...
각 Stage에서:
0. S4 시작 전: 3종 산출물 작성 순서를 Human에게 간략 보고 (승인 대기 아님, 즉시 Wave 1 진행)
...
6. 게이트 유형에 따라:
   - **[STOP]** (S2, S3): Human에게 리뷰 요청 → 승인 대기
   - **[AUTO-PASS]** (S1, S4): 알림 출력 → 즉시 다음 Stage 진행
...
1. **`bash scripts/sigil-gate-check.sh {project} {stage}` 실행** — DoD [AI] 항목 자동 검증
2. PASS → 알림 출력 (`✅ {Stage} Gate AUTO-PASS: {DoD 요약}`) → 다음 Stage 즉시 진행 (Human 응답 대기 없음)
3. FAIL → **[STOP]으로 에스컬레이션** — Human에게 FAIL 사유 보고 + 승인 대기
4. gate-log에 `AUTO` 결과 기록

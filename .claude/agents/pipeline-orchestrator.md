...
각 Stage에서:
0. S3/S4 시작 전 Intra-Stage Gate 실행: 구조/접근 방식을 Human에게 제시하고 승인 후 진행
...
Wave 1 (순차): technical-writer → 7대 산출물 초안 작성
  - 상세 기획서, 사이트맵, 로드맵, 개발 계획, WBS, UI/UX 기획서, 테스트 전략서
...
4. 라운드 1 결과를 `{folderMap.templates}/council-evaluation-template.md` 기반으로 기록
...
1. 기획 패키지 산출물 존재 확인:
   ...
   - 사이트맵: `{folderMap.product}/{project}/YYYY-MM-DD-s4-sitemap.md`
   - 로드맵: `{folderMap.product}/{project}/YYYY-MM-DD-s4-roadmap.md`
   - 개발 계획: `{folderMap.product}/{project}/YYYY-MM-DD-s4-development-plan.md`
   - WBS: `{folderMap.product}/{project}/YYYY-MM-DD-s4-wbs.md`

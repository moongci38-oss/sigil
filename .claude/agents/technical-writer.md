...
description: |
  ...
  S3 기획서(PRD/GDD) 기반으로 4종 산출물 초안을 작성하고,
...
S4 기획 패키지 4종 산출물을 S3 기획서 기반으로 작성한다. "개발자가 읽고 바로 구현 가능한 수준"이 목표.
...
| 3 | 개발 계획 | `*-s4-development-plan.md` | 기술 스택 + C4 아키텍처 + ADR + 로드맵 + WBS + Trine 세션 로드맵 |
| 4 | 테스트 전략서 | `*-s4-test-strategy.md` | 테스트 피라미드 + FE/BE 도구 + 시딩 전략 + 커버리지 목표 |
...
4. **[3] 개발 계획**: [1]+[2] 기반 기술 스택 → C4 아키텍처 → ADR → 로드맵 → WBS → Trine 세션 로드맵
5. **[4] 테스트 전략서**: [1]+[3] 기반 테스트 계층 → FE/BE 도구 → 시딩 전략 → 커버리지 목표

## 관리자 페이지 포함 시

S3에 관리자 기능이 있으면:
- [1] 상세 기획서, [2] UI/UX 기획서: 서비스 + 관리자 **각각** 작성
- [3] 개발 계획, [4] 테스트 전략서: 통합 문서에 관리자 섹션 병기

## Wave 역할

| Wave | 역할 |
|------|------|
| **W1** | 4종 산출물 초안 작성 (본 에이전트 담당) |
